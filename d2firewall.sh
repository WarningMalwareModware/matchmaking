...
install_dependencies() {
  ...
  if [ "$yn" == "y" ]; then
    ...
    sudo DEBIAN_FRONTEND=noninteractive apt-get -y -q install iptable iptables-persistent ngrep > /dev/null
  fi
}
...
if [ "$action" == "setup" ]; then
  ...
  if ! sudo iptables-save | grep -q "REJECT"; then 
    ...
    pos=$(iptables -L FORWARD | grep "system" | tail -1)
    reject=$(<reject.rules)
  ...
  sleep 1
  if [ $platform == "psn" ]; then 
    ngrep -q -W byline -d $INTERFACE "psn-4" udp | grep --line-buffered -o -P 'psn-4 [0]{8}\K[A-F0-9]{7}' | tee -a data.txt &
  elif [ $platform == "xbox" ]; then
    ngrep -q -W byline -d $INTERFACE "xbox-" udp | grep --line-buffered -o -P 'xbox-4 [0]{8}\K[A-F0-9]{7}' | tee -a data.txt &
  elif [ $platform == "steam" ]; then
    ngrep -q -W byline -d $INTERFACE "steamid:" udp | grep --line-buffered -o -P 'steamid:76561\K[0-9]{12}' | tee -a data.txt &
  fi
  ...
  # remove duplicates
  awk '!a[$0]++' /tmp/data.txt > /tmp/temp.txt && mv /tmp/temp.txt ./data.txt
  ...
  echo -e "${RED}Additional blocking of the SDR server is now activated.${NC}"
